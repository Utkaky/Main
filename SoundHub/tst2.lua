...
-- Icons
	...
	local function LoadLucideIcons()
		...
		if Success and type(Module) == "table" and type(Module.GetAsset) == "function" then
			return Module
		end

		local LegacySuccess, LegacyModule = pcall(function()
			return loadstring(game:HttpGet("https://pastefy.app/5oEQ50tx/raw"))()
		end)

		if LegacySuccess and type(LegacyModule) == "table" then
			return LegacyModule.assets or LegacyModule
		end
	...
	local function GetLucideIcon(IconName)
		...
		if type(LucideIcons) == "table" then
			if type(LucideIcons.GetAsset) == "function" then
				local IconData = LucideIcons:GetAsset(IconName)
				if IconData ~= nil then
					return IconData
				end
			end

		if LucideIcons[IconName] ~= nil then
			return LucideIcons[IconName]
		end

		if LucideIcons["lucide-"..IconName] ~= nil then
...
-- Create Elements
	...
	local function ApplyIcon(Object, IconData)
		local ResolvedIcon = GetOrionIcon(IconData) or GetLucideIcon(IconData)

		if type(ResolvedIcon) == "table" and ResolvedIcon.Url ~= nil then
			Object.Image = ResolvedIcon.Url
			Object.ImageRectOffset = ResolvedIcon.ImageRectOffset or Vector2.zero
			Object.ImageRectSize = ResolvedIcon.ImageRectSize or Vector2.new(24, 24)
			return Object
		end

		if ResolvedIcon ~= nil then
			Object.Image = ResolvedIcon
			Object.ImageRectOffset = Vector2.zero
			Object.ImageRectSize = Vector2.new(0, 0)
		elseif IconData ~= nil then
	...
	CreateElement("Image", function(ImageID)
		local ImageNew = Create("ImageLabel", {BackgroundTransparency = 1})
		return ApplyIcon(ImageNew, ImageID)
	end)

	CreateElement("ImageButton", function(ImageID)
		local Image = Create("ImageButton", {BackgroundTransparency = 1})
		return ApplyIcon(Image, ImageID)
	end)
...
function OrionLib:MakeWindow(WindowConfig)
    ...
    DropdownContainer.CanvasSize =
        UDim2.new(0,0,0,DropdownList.AbsoluteContentSize.Y)

    if Dropdown.Toggled then
        local NewSize =
            #Dropdown.Options > MaxElements
